...
class SimpleNet(nn.Module): # nn.Module - базовый класс для всех нейронных сетей в PyTorch
    def __init__(self, input_size=5, hidden_size=10, output_size=1): # Нейронная сеть с входным слоем из 5 признаков, скрытым слоем из 10 нейронов и выходным слоем из 1 нейрона (регрессия)
...
X_train = torch.randn(100, 5) # 100 объектов по 5 признаков
y_train = (X_train.sum(dim=1, keepdim=True) ** 2) + 3.0 

criterion = nn.MSELoss() # Функция потерь для регрессии

optimizer = optim.Adam(model.parameters(), lr=0.01) # Оптимизатор optim.Adam(веса, скорость обучения). Оптимизатор нужен для того, чтобы корректировать веса модели на основе градиентов, посчитанных в loss.backward()

epochs = 200  # Сколько раз модель просмотрит данные(пройдет по всем объектам)
